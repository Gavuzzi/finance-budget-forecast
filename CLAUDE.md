...
- `tools/lifecycle-test.js` — drives every WRITE path against the live backend (sign-in → org → seed → SIE import → planning edits → scenario → budget lock → report → mobile). `tests/e2e.js` runs in demo mode where writes are stubbed, so it can only prove UI behaviour; this proves data actually saves. Writes to production — read its header before running
