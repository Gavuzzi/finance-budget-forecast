...
- [ ] `[B]` **Security pass** *(HANDOFF-fixes Task 1)* — (a) **XSS:** the app builds UI via `innerHTML` and interpolates user-typed reporting-line names + Fortnox counterparty names unescaped → a name like `<img src=x onerror=…>` executes. Add `escapeHtml()` (lib.js) at every injection point. Same-org blast radius (RLS) but an editor could hit a viewer — a genuine finding, and the concrete start of the "security review" compliance item. (b) **Password reset** — no recovery today; add "Forgot password?" → `sb.auth.resetPasswordForEmail`. (c) **Adversarial RLS audit** — confirm org A truly can't read org B across every tenant table incl. Phase 5 additions; record result in TESTING.md.
